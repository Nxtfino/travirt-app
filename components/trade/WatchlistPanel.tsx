...
import { Stock, WatchlistGroup, TransactionType, OrderType, WatchlistSettings, SortByType, Watchlist } from '../../types';
...
const NotesEditor: React.FC<{
    ...
    stockSymbol: string;
    onClose: () => void;
}> = ({ watchlistId, stockSymbol, onClose }) => {
...
}> = ({ settings, onSettingsChange, onSort }) => {
        ...
        <div className="p-3 my-2 border border-overlay rounded-lg bg-base animate-fade-in space-y-3">
            <div>
                ...
                <input
                    ...
                    autoFocus
                    className="w-full bg-overlay border border-gray-600 rounded-md py-1.5 px-3 text-sm text-text-primary focus:ring-1 focus:ring-primary"
                />
            </div>
            <div className="flex items-center gap-2">
                {colors.map((color, index) => (
                    <button
                        ...
                        className={`w-5 h-5 rounded-full transition-transform transform hover:scale-110 ${selectedColor === color ? 'ring-2 ring-offset-2 ring-offset-base ring-white' : ''} ${index === 0 ? 'border-2 border-gray-400' : ''}`}
                        style={{ backgroundColor: index === 0 ? 'transparent' : color }}
                        aria-label={`Select color ${color}`}
                    />
                ))}
            </div>
            <div className="flex justify-end items-center gap-2">
                <button onClick={handleCreateClick} className="px-4 py-1.5 text-sm font-semibold text-white bg-primary rounded-md hover:bg-primary-focus transition disabled:opacity-50" disabled={!name.trim()}>Create</button>
                <button onClick={onCancel} className="px-4 py-1.5 text-sm font-semibold text-text-secondary bg-overlay rounded-md hover:bg-base transition">Close</button>
            </div>
        </div>
    );
};
...
}> = ({ stock, settings, isDiscover, hasNote, holdingQty, onSelect, onOrder, onRemove, onDragStart, onDragEnter, onDragEnd, onMouseEnter, onMouseLeave, isExpanded, onDepthClick, onMoreClick }) => {
...
const EditGroupForm: React.FC<{
    ...
    return (
        <div className="p-2 space-y-3 bg-base border border-overlay rounded-b-md animate-fade-in" onClick={e => e.stopPropagation()}>
            <input
                type="text"
                value={name}
                onChange={(e) => setName(e.target.value)}
                autoFocus
                className="w-full bg-overlay border border-gray-600 rounded-md py-1.5 px-3 text-sm text-text-primary"
            />
            <div className="flex items-center gap-2">
                 <button
                    type="button"
                    onClick={() => setSelectedColor(defaultColor)}
                    className={`w-5 h-5 rounded-full transition-transform transform hover:scale-110 flex items-center justify-center bg-grid-pattern ${selectedColor === defaultColor ? 'ring-2 ring-offset-2 ring-offset-base ring-white' : ''}`}
                    style={{ 
                ...
                {colors.map((color) => (
                    <button
                        ...
                        type="button"
                        onClick={() => setSelectedColor(color)}
                        className={`w-5 h-5 rounded-full transition-transform transform hover:scale-110 ${selectedColor === color ? 'ring-2 ring-offset-2 ring-offset-base ring-white' : ''}`}
            ...
            <div className="flex justify-end gap-2">
                <button onClick={handleUpdate} className="px-4 py-1.5 text-sm font-semibold text-white bg-primary rounded-md hover:bg-primary-focus transition">Update</button>
                <button onClick={onClose} className="px-4 py-1.5 text-sm font-semibold text-text-secondary bg-overlay rounded-md hover:bg-base transition">Close</button>
            </div>
        </div>
    );
};
...
// Modals
const MoveGroupModal: React.FC<{ group: WatchlistGroup; onClose: () => void; onMove: (targetWatchlistId: number) => void; panelRef: React.RefObject<HTMLDivElement> }> = ({ group, onClose, onMove, panelRef }) => {
    const { watchlists, activeView } = useWatchlist();
    ...
    return createPortal(
        <div className="fixed inset-0 bg-black bg-opacity-85 z-50 animate-fade-in" onClick={onClose}>
            <div ref={modalRef} style={style} className="bg-surface rounded-lg shadow-2xl w-full max-w-sm p-6 space-y-4" onClick={e => e.stopPropagation()}>
                <h3 className="text-xl font-bold text-text-primary">Move to lists</h3>
                <p className="text-sm text-muted">
                    Move group - <span className="font-semibold text-text-primary">{group.name}</span> from current watchlist - <span className="font-semibold text-text-primary">{currentWatchlist?.name}</span> to
                </p>
                <select 
                    value={targetId} 
                ...
                <div className="flex justify-end gap-2">
                    <button onClick={onClose} className="px-4 py-2 text-sm font-semibold rounded-md bg-overlay hover:bg-base text-text-secondary hover:text-text-primary transition-colors">Cancel</button>
                    <button onClick={handleMove} className="px-4 py-2 text-sm font-semibold rounded-md bg-primary hover:bg-primary-focus text-white">Move</button>
                </div>
            </div>
        </div>,
        document.body
...
const SortGroupModal: React.FC<{ group: WatchlistGroup; onClose: () => void; onSort: (sortBy: '%' | 'LTP' | 'A-Z' | 'EXCH') => void; panelRef: React.RefObject<HTMLDivElement> }> = ({ group, onClose, onSort, panelRef }) => {
    ...
    return createPortal(
        <div className="fixed inset-0 bg-black bg-opacity-85 z-50 animate-fade-in" onClick={onClose}>
            <div ref={modalRef} style={style} className="bg-surface rounded-lg shadow-2xl w-full max-w-xs p-6 space-y-4" onClick={e => e.stopPropagation()}>
                ...
                <div className="grid grid-cols-2 gap-2">
                    {sortOptions.map(opt => (
                        <button 
                            ...
                            className={`px-4 py-2 text-sm font-semibold rounded-md border transition-colors ${sortBy === opt.key ? 'bg-primary text-white border-primary' : 'bg-base text-muted border-gray-600 hover:border-primary hover:text-text-primary'}`}
                        >
                            {opt.label}
                        </button>
                    ))}
                 ...
                 <div className="flex justify-center gap-2 pt-2">
                    <button onClick={handleSave} className="px-6 py-2 text-sm font-semibold rounded-md bg-primary hover:bg-primary-focus text-white">Save</button>
                    <button onClick={onClose} className="px-6 py-2 text-sm font-semibold rounded-md bg-overlay hover:bg-base text-text-secondary hover:text-text-primary transition-colors">Close</button>
                </div>
            </div>
        </div>,
        document.body
...
const WatchlistPanel: React.FC<WatchlistPanelProps> = ({ activeList, isDiscover, selectedStock, onStockSelect, onOrderAction, onCreateGTT, onCreateAlert, onShowMarketDepthModal }) => {
    ...
    const [moveModalState, setMoveModalState] = useState<{ isOpen: boolean, group: WatchlistGroup | null }>({ isOpen: false, group: null });
    const [sortModalState, setSortModalState] = useState<{ isOpen: boolean, group: WatchlistGroup | null }>({ isOpen: false, group: null });
    const [activeGroupMenu, setActiveGroupMenu] = useState<string | null>(null);

    const { addStockToGroup, removeStockFromGroup, reorderStockInGroup, reorderGroups, toggleGroupCollapse, toggleGroupMaximize, updateGroup, addGroup, pinStock, removeGroup, moveGroupToWatchlist, updateGroupSymbols, updateWatchlistSettings, sortAllAssetsInWatchlist, addWatchlistFromDiscover } = useWatchlist();
        ...
        const handleKeyDown = (event: KeyboardEvent) => {
            ...
            if (stock && !isDiscover) {
                if (event.key.toLowerCase() === 'b') {
                    event.preventDefault();
                    onOrderAction({ stock: stock, type: TransactionType.BUY });
                } else if (event.key.toLowerCase() === 's') {
                    event.preventDefault();
                    onOrderAction({ stock: stock, type: TransactionType.SELL });
    ...
    useEffect(() => {
        const handleKeyDown = (event: KeyboardEvent) => {
            ...
            if (event.key === 'Escape') {
                ...
                setActiveGroupMenu(null);
                setMoveModalState({ isOpen: false, group: null });
                setSortModalState({ isOpen: false, group: null });
    ...
    const handleMoreClick = (symbol: string, event: React.MouseEvent<HTMLButtonElement>) => {
        ...
        setActiveMoreMenu(prev => 
            prev?.symbol === symbol ? null : { symbol, triggerEl: target }
        );
    };
    ...
    return (
        <div ref={panelRef} className="flex flex-col h-full">
            ...
            <div className="shrink-0">
                <div className="p-3 border-b border-overlay">
                    <div className="relative z-30" ref={searchContainerRef}>
                        ...
                        {query && !isDiscover && (
                            <div ref={searchResultsRef} className="absolute top-full left-0 right-0 mt-1 bg-base border border-overlay rounded-md shadow-lg z-20 max-h-60 overflow-y-auto custom-scrollbar">
                                {searchResults.length > 0 ? (
                                    searchResults.map(stock => (
                                        <div key={stock.symbol} className="p-2 hover:bg-overlay flex items-center justify-between">
                                            <div>
                                            ...
                                            <Tooltip title={activeGroupId ? `Add to '${activeList.groups.find(g => g.id === activeGroupId)?.name}'` : 'Select a group first'}>
                                                <div> {/* div wrapper needed for tooltip on disabled button */}
                                                    <button 
                                                        onClick={() => {
                                                            if (activeGroupId) {
                                                        ...
                                                        aria-label={`Add ${stock.symbol} to active group`}
                                                    >
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-2 pt-0 min-h-0" ref={scrollContainerRef}>
                                ...
                                {!isDiscover &&
                                    <div className="flex items-center gap-3 text-muted text-xs">
                                        ...
                                        <Tooltip title="Edit group">
                                            <button onClick={(e) => { e.stopPropagation(); setEditingGroupId(isEditing ? null : group.id); }}>
                                                    ...
                                                    <ul>
                                                        <li>
                                                            <Tooltip title="Move group to another list">
                                                                <button onClick={() => { setMoveModalState({ isOpen: true, group }); setActiveGroupMenu(null); }} className="w-full text-left flex items-center gap-2 px-3 py-2 hover:bg-overlay text-sm">
                                                        ...
                                                        <li>
                                                            <Tooltip title="Sort stocks in this group">
                                                                <button onClick={() => { setSortModalState({ isOpen: true, group }); setActiveGroupMenu(null); }} className="w-full text-left flex items-center gap-2 px-3 py-2 hover:bg-overlay text-sm">
                                                        ...
                                                        <li>
                                                            <Tooltip title="Delete this group">
                                                                <button onClick={() => { if (window.confirm(`Are you sure you want to delete the group "${group.name}"?`)) { removeGroup(activeList.id, group.id); } setActiveGroupMenu(null); }} className="w-full text-left flex items-center gap-2 px-3 py-2 hover:bg-overlay text-sm text-danger">
                            ...
                            {!group.isCollapsed && (
                                <div onDragOver={(e) => e.preventDefault()}>
                                    {group.symbols.map((symbol, index) => {
                                        ...
                                        return (
                                             <div key={stock.symbol} className={`rounded-lg transition-colors duration-200 ${isExpanded || isNotesOpen ? 'bg-base shadow-lg' : 'hover:bg-overlay/50'}`}>
                                                <StockRow
                                                    stock={stock}
                                                    settings={activeList.settings}
                                                    isDiscover={isDiscover}
                                                    holdingQty={holdingsMap.get(stock.symbol)}
                                                    hasNote={hasNote}
                                                    onSelect={() => onStockSelect(stock)}
                                                    onOrder={(type) => onOrderAction({ stock, type })}
                                                    onRemove={() => removeStockFromGroup(activeList.id, group.id, stock.symbol)}
                                                    ...
                                                    onDepthClick={() => setExpandedSymbol(isExpanded ? null : stock.symbol)}
                                                ...
                                                {isNotesOpen && (
                                                    <NotesEditor
                                                        ...
                                                        onClose={() => setNotesOpenForSymbol(null)}
                                                    />
                                                )}
                                            </div>
                                        );
             ...
             {activeMoreMenu && (
                 <MoreOptionsMenu
                    ...
                    onShowNotes={() => handleToggleNotes(activeMoreMenu.symbol)}
                    onShowMarketDepthModal={() => onShowMarketDepthModal(activeMoreMenu.symbol)}
                />
            )}
            {moveModalState.isOpen && moveModalState.group && (
                <MoveGroupModal
                    group={moveModalState.group}
                    panelRef={panelRef}
                    onClose={() => setMoveModalState({ isOpen: false, group: null })}
                    onMove={(targetWatchlistId) => {
            ...
            {sortModalState.isOpen && sortModalState.group && (
                <SortGroupModal
                    group={sortModalState.group}
                    panelRef={panelRef}
                    onClose={() => setSortModalState({ isOpen: false, group: null })}
                    onSort={(sortBy) => {
